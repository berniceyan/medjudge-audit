...
def example_score(g: pd.DataFrame) -> float:
    """g: the grade rows for ONE (prompt_id, response_model, judge, variant, run_tag). 
    Needs columns: grade (bool), points (int)."""
    g = g[g.grade.notna()]
...
def per_example_scores(b: pd.DataFrame) -> pd.DataFrame:
    ...
    return (b.groupby(keys).apply(example_score, include_groups=False)
